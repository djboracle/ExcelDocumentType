...
BEGIN
  ...
  -- Define Styles (Optional)
   ...
   -- All Names beginning with 'Ba' will be purple.
   
   v_condition_array            := ExcelDocTypeUtils.CONDITIONS_TABLE();
   
   v_condition_rec              := NULL;
   v_condition_rec.value        := 'NOT(ISERROR(SEARCH(&quot;Ba&quot;,RC)))';
   v_condition_rec.format_style := 'color:purple';

   ExcelDocTypeUtils.addConditionType(v_condition_array,v_condition_rec);

   -- Format range for Column 1 starting at row 2 and going to row 65000 ... 
   v_conditional_format_rec            := NULL;
   v_conditional_format_rec.range      := 'R2C1:R65000C1'; 
   v_conditional_format_rec.conditions := v_condition_array;
   ...
   -- This will generate a dynamic column header list based on the query's SELECT clause ... good to use if the query is SELECT * FROM some_table ...
   ...
   excelReport.displayDocument('TestEmployeeReport.xls');
